...
stackca_clim <- c(dem, aspect, TRI, popdens, lc, road_dens, road_dist, streams_dist, urban_dist, rec_dist, forest_dist, forest_dens, dist_fire_1yr, dist_fire_23yrs, dist_fire_45yrs, drought)
...
library(raster)
#any categorical variables used need to be identified first
f <- list(terra::levels(stackca_clim$category)) 
names(f) <- "landcover"

raster::predict(object = stackca_clim, model = climate_model_allyears, fun = predict, factors = f,ext=ext, progress= "windows")
```
