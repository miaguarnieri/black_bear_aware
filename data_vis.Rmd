...
#remove perennial ice and snow to make plot look more reasonable
coeffs_tidy_nosnow <- coeffs_tidy %>% 
  filter(rownames_coeffs != "lcperrenial_ice_snow")

#test plot
ggplot(coeffs_tidy_nosnow, aes(x = estimate, y = rownames_coeffs)) + 
  geom_point() +
...
bluevar <- coeffs_tidy_nosnow[c(2:12, 18), ] %>% 
  mutate(rownames_coeffs = c("Elevation", "High-Intensity Development", "Low-Intensity Development", "Medium-Intensity Development", "Developed Open Space", "Forest", "Grassland/Herbaceous", "Open Water", "Planted/Cultivated", "Shrub/Scrub", "Wetlands", "Terrain Ruggedness"))

greenvar <- coeffs_tidy_nosnow[c(13, 17), ] %>% 
  mutate(rownames_coeffs = c("Distance to Forest", "Distance to Streams"))

grayvar<- coeffs_tidy_nosnow[c(14:16, 19), ] %>% 
  mutate(rownames_coeffs = c("Population Density", "(Population Density)^2", "Distance to Recreational Areas", "Distance to Urban Areas"))
...
brownvar <- coeffs_tidy_nosnow[c(23), ] %>% 
  mutate (rownames_coeffs = c("Drought"))
...
#nice plot
ggplot() + 
  geom_point(bluevar, mapping = aes(x = estimate, y = rownames_coeffs), color = "#528faa") +
  geom_pointrange(bluevar, mapping = aes(x = estimate, y = rownames_coeffs, xmin = estimate-std_error, xmax = estimate+std_error), color = "#528faa") +
  geom_point(greenvar, mapping = aes(x = estimate, y = rownames_coeffs), color = "#89a676") +
  geom_pointrange(greenvar, mapping = aes(x = estimate, y = rownames_coeffs, xmin = estimate-std_error, xmax = estimate+std_error), color = "#89a676") +
  geom_point(grayvar, mapping = aes(x = estimate, y = rownames_coeffs), color = "#c3c2b1") +
  geom_pointrange(grayvar, mapping = aes(x = estimate, y = rownames_coeffs, xmin = estimate-std_error, xmax = estimate+std_error), color = "#c3c2b1") +
  geom_point(orangevar, mapping = aes(x = estimate, y = rownames_coeffs), color = "#ffbd59") +
  geom_pointrange(orangevar, mapping = aes(x = estimate, y = rownames_coeffs, xmin = estimate-std_error, xmax = estimate+std_error), color = "#ffbd59") +
  geom_point(brownvar, mapping = aes(x = estimate, y = rownames_coeffs), color = "#b68a67") +
  geom_pointrange(brownvar, mapping = aes(x = estimate, y = rownames_coeffs, xmin = estimate-std_error, xmax = estimate+std_error), color = "#b68a67") +
  geom_point(intercept, mapping = aes(x = estimate, y = rownames_coeffs)) +
  geom_pointrange(intercept, mapping = aes(x = estimate, y = rownames_coeffs, xmin = estimate-std_error, xmax = estimate+std_error)) +
  scale_x_continuous(breaks = seq(-3, 4, 0.5)) +
