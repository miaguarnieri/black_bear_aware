...
popdens_sq <- popdens^2

proj_conf <- rast(here(gdrive_data, "/AnalysisData/model_outputs/projected_mod3_clim_map_squared.tif"))

pop_stack <- c(proj_conf, popdens, popdens_sq)

pop_conf_df <- values(pop_stack, dataframe = TRUE, na.rm = TRUE)

colnames(pop_conf_df) <- c("pred_conf", "popdens", "popdens_sq")

ggplot(pop_conf_df) +
  #geom_point(aes(x = popdens, y = pred_conf)) +
  geom_point(aes(x = popdens_sq, y = pred_conf))
...
mep(sq_mod3climate, which = c("popdens", "popdens"))
