...
stack17 <- c(dem, aspect, ter_rug, pop17, lc16, road_dens, road_dist, urban_dist_16,rec_dist,fc_dist_16, fdens)
...
#sampling random points from this misshapen california shpfile
sample19 <- st_sample(cabuff19_inv_sf, 1364) %>% 
  st_as_sf()
...
#nonconflict points
zero_vars19 <- extract(stack21, zeros21_formatted, method = "simple", bind = TRUE, ID = TRUE) %>% 
  mutate(ID = 0)
...
con_vars21 <- extract(stack16, con_union16, method = "simple", bind = TRUE, ID = TRUE)

#combine conflict and non-conflict

model_df_21 <- rbind(zero_vars16, con_vars16)
