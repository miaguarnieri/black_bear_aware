...
con_buff16 <- project(con_16_sv,crs(ca_raster))

con_buff16_sf <- st_as_sf(con_buff16)
...
conbuff16_inverse <- mask(ca_raster, con_buff16, inverse = TRUE)
...
cabuff16_inv_sf <- st_as_sf(conbuff16_inv_pol)

#sampling random points from this misshapen california shpfile
sample16 <- st_sample(conbuff16_inv_sf, 427)
...
intersect16 <- st_join(sample16_buff, con_buff16_sf, join = st_intersects, left = FALSE)
...
zeros16 <- ms_erase(target = sample16_buff, erase = intersect16, remove_slivers = FALSE)
...
conflict16_model <- rbind(con_16, zeros16_formatted) %>% 
  st_union(by_feature = TRUE)

con16_model_sv <- vect(conflict16_model)
...
var_ex16 <- extract(stack16, conflict, method = "simple", bind = TRUE, ID = TRUE)

var_df16 <- as.data.frame(var_ex16)

vars16 <- colnames(var_df16)
