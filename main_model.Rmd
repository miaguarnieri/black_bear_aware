...
conflict_points <- vect(here(gdrive_data,"/IntermediateData/IntermediateData_Files/2022_07_18_WIRClean_CDFW.csv"))
...
dem_ca_cropped <- mask(x = dem_ca, mask = ca_raster)

#plot DEM raster
tm_shape(dem_ca_cropped) +
  tm_raster() +
  tmap_mode("view")
...
m <- rbind(c("Open Water", "open_water"),
       c("Perennial Snow/Ice", "perennial_ice_snow"),
       c("Developed, Open Space", "developed_open_space"), 
       c("Developed, Low Intensity", "developed_low_int"),
       c("Developed, Medium Intensity", "developed_med_int"),
       c("Developed, High Intensity", "developed_high_int"),
       c("Barren Land", "barren_land"),
       c("Deciduous Forest", "forest"),
       c("Evergreen Forest", "forest"),
       c("Mixed Forest", "forest"),
       c("Shrub/Scrub", "shrubland"),
       c("Hay/Pasture", "planted_cultivated"),
       c("Cultivated Crops", "planted_cultivated"),
       c("Woody Wetlands", "wetlands"),
       c("Emergent Herbaceous Wetlands", "wetlands"))

#reclassification function

nlcd_2001_reclassed <- classify(land_cover_2001, m)
